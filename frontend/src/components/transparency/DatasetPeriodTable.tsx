  ...
  return (
    <>
      <div className="overflow-x-auto">
        {/* 12px cells / 11px header / 13px row name — the frame's table, normalised
            off its 1.354808 scale (see `TransparencySection`). */}
        <table className="w-full min-w-[720px] text-left text-xs">
          <caption className="sr-only">자료별 기준 기간과 표시 개수</caption>
          <thead>
            {/* Figma rules the header off with a 2px brand line and separates the
                rows with a hairline — the header is the only heavy rule. */}
            <tr className="border-b-2 border-primary text-[11px] font-bold text-ink-subtle">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.name} className="border-b border-hairline align-top last:border-b-0">
                <th scope="row" className="py-2.5 pr-3 text-left text-[13px] font-bold text-ink">
                  {row.name}
                  {row.note ? (
                    <span className="mt-1 block text-[11px] font-normal text-ink-subtle">
                      {row.note}
                    </span>
                  ) : null}
                </th>
                ...
                <td className="py-2.5 pr-3">
                  {/* Text carries the meaning; the tint is the supporting signal.
                      Figma shortens the derived label to 계산값; the served wording
                      공식 자료 기반 계산값 is kept because it names WHAT the value was
                      calculated from, which is the distinction this table exists to
                      make (and both strings are compared exactly by two suites). */}
                  <DataStatusBadge status={row.valueKind} label={VALUE_KIND_LABELS[row.valueKind]} />
                </td>
                <td className="py-2.5 pr-3 tabular-nums text-ink-muted">{row.referencePeriod}</td>
                {/* The COUNT of records actually loaded. Figma prints a flat
                    `32개 지역` on every row and `자료 없음` for 처리시설; production
                    serves a different count per dataset and does hold facility
                    records, so both are ignored in favour of the served figures. */}
                <td className="py-2.5 pr-3 tabular-nums text-ink-muted">{formatCount(row.count)}</td>
                <td className="py-2.5 pr-3 text-ink-muted">{row.coverage}</td>
              </tr>
            ))}
          </tbody>
        </table>
